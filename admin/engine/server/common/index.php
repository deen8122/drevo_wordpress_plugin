...
require_once "{$CURFLD}constants.php";
require_once "{$CURFLD}variables.php";
require_once "{$CURFLD}strings.php";
require_once "{$CURFLD}config.php";
require_once "{$CURFLD}sql.php";
require_once "{$CURFLD}trigger.php";
...
function getData2($rubric_id, $orderby = '', $limit = '', $features = array(), $fvalues = false, $uslovia = array())
{
	...
	{
		$res_feat = $database->query("
				SELECT ".DB_PREFIX."features.ID_FEATURE, feature_type, feature_text
				FROM ".DB_PREFIX."features NATURAL JOIN ".DB_PREFIX."rubric_features
				WHERE ID_RUBRIC=".$rubric_id." and feature_deleted=0
				ORDER BY rubricfeature_pos
			");
		while (list($feature_id,$ftype,$fname) = mysql_fetch_array($res_feat))
		{
			$features[$feature_id] = array($ftype,$fname);
		}
	}
	$add_tbl = '';
	$add_sql = '';
	...
	while (list($good_id, $url) = mysql_fetch_array($res))
	{
		...
		foreach ($features as $feature_id=>$fprms)
		{
			...
			{
				if ($fvalues)
					$data[$good_id][$feature_id] = array($fprms[1],getFeatureValue($good_id, $feature_id));
				else
					$data[$good_id][$feature_id] = array($fprms[1],getFeatData($feature_id, $good_id, true, $fprms[0]));
			}
		}
	}
	return $data;
...
function getFeatData($feat_id, $good_id, $ppl = true, $restype=0)
{
	global $database;

	if($restype>0) list($resfeat) = $database->getArrayOfQuery("
			SELECT goodfeature_value
			FROM ".DB_PREFIX."goods_features 
			WHERE ID_FEATURE = '".$feat_id."' and ID_GOOD = '".$good_id."'");
	else list($restype, $resfeat) = $database->getArrayOfQuery("
